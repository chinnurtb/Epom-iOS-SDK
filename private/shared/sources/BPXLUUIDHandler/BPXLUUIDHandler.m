...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
	return [ESUtilsPrivate deviceMACAdressSHA1];
...
+ (NSString *)storeUUID:(BOOL)itemExists
{
	...
	if (status != noErr)
	{
		ES_LOG_ERROR(@"BPXLUUIDHandler Keychain Save Error: %ld", status);
		uuid = nil;
	}
...
+ (NSString *)UUID
{
	...
	if (status != noErr) 
	{
		...
		{
			ES_LOG_ERROR(@"BPXLUUIDHandler Unhandled Keychain Error %ld", status);
			return nil;
		}
	}
...
+ (void)reset
{
	...
	if (status != noErr)
	{
		ES_LOG_ERROR(@"BPXLUUIDHandler Keychain Delete Error: %ld", status);
	}
	CFRelease(query);
}
